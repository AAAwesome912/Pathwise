...
const RegisterPage = () => {
  ...
  const [isVerificationModalOpen, setIsVerificationModalOpen] = useState(false); // New state to control modal visibility
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Sending registration data for verification:", payload);
      const response = await axiosInstance.post('/api/auth/register', payload);

      if (response.status === 200) {
        setIsVerificationModalOpen(true); // Open the verification modal on success
        toast.success(`✅ A verification code has been sent to ${payload.email}.`);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-400 to-teal-500 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-lg w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
            ...
            <div className="flex items-center justify-between mt-6 sm:col-span-2">
      ...
      {/* This is the Verification Modal component */}
      <VerificationModal
        isOpen={isVerificationModalOpen}
        onClose={() => setIsVerificationModalOpen(false)}
        email={formData.email} // Pass the email to the modal
