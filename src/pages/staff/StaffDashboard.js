...
const StaffDashboard = () => {
  ...
  You are assigned to the <span className="font-semibold text-blue-600">{user?.office || 'Unknown Office'}</span>
  {user?.office === 'Registrar' && (
    <> <span className="font-semibold text-blue-600">{user?.windowNo}</span></>
  )}
  {user?.office === 'Library' && (
    <> <span className="font-semibold text-blue-600">{user?.section}</span></>
  )}
  {user?.office === 'Departmental' && (
