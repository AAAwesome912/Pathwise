...
const QueueManagementPage = () => {
  ...
  // Add border-b, border-gray-200, and pb-4 (padding-bottom)
  <div className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200 border-b pb-1 mb-1">
...
return (
    <div className="bg-white p-4 md:p-8 rounded-xl shadow-xl">
        ...
        {Object.values(windows).every(
            ...
            <>
                {/* --- Correct rendering logic starts here --- */}

                {/* Section for Currently Serving */}
                <h2 className="text-xl font-bold text-green-600 mb-3">Currently Serving</h2>
                <div className="space-y-4">
                    {Object.values(windows)
                        .flatMap(buckets => buckets.in_progress)
                        .map(t => <TicketCard key={t.id} t={t} />)}
                </div>

                {/* Section for Called Tickets */}
                <h2 className="text-xl font-bold text-orange-600 mt-6 mb-3">Called</h2>
                <div className="space-y-4">
                    {Object.values(windows)
                        .flatMap(buckets => buckets.called)
                        .map(t => <TicketCard key={t.id} t={t} showServe />)}
                </div>

                {/* Section for Waiting Tickets */}
                <h2 className="text-xl font-bold text-blue-600 mt-6 mb-3">Waiting</h2>
                <div className="space-y-4">
                    {Object.values(windows)
                        .flatMap(buckets => buckets.waiting)
                        .sort((a, b) => {
                            if (a.priority && !b.priority) return -1;
                            if (!a.priority && b.priority) return 1;
                            return a.office_ticket_no - b.office_ticket_no;
                        })
                        .map(t => <TicketCard key={t.id} t={t} showCall />)}
                </div>

                {/* --- Correct rendering logic ends here --- */}
            </>
        )}
