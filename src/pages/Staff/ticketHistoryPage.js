...
const TicketHistoryPage = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/tickets/office/${user.office}`)
  ...
  const filtered = tickets.filter(t => {
    ...
    const matchesWindow = windowFilter === 'all' || String(t.window_no ?? 'Unassigned') === windowFilter;
    const matchesDate = !dateFilter || new Date(t.created_at).toISOString().slice(0, 10) === dateFilter;
    return matchesStatus && matchesService && matchesWindow && matchesDate;
  });
  ...
  return (
    <div className="bg-white p-4 md:p-8 rounded-xl shadow-xl flex flex-col md:flex-row gap-6">
      {/* Filter Sidebar */}
